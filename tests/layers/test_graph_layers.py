...
@pytest.mark.parametrize('layer',
                         [nfp.EdgeUpdate, nfp.NodeUpdate, nfp.GlobalUpdate])
def test_masking(inputs_no_padding, inputs_with_padding, smiles_inputs, layer):
    ...
    assert np.all(
        np.isclose(update_state_global, update_state_global_pad, atol=1E-4))


def test_masking_message(inputs_no_padding, inputs_with_padding,
                         smiles_inputs):
    preprocessor, inputs = smiles_inputs
